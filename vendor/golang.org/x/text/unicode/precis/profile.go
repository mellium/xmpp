...
// Transform satisfies the Transformer interface and performs the PRECIS
// enforcement step on the given UTF-8 encoded bytes.
func (p Profile) Transform(dst, src []byte, atEOF bool) (nDst, nSrc int, err error) {
	...
	c := transform.Chain(ts...)
	nDst, nSrc, err = c.Transform(dst, src, atEOF)
	if !atEOF && err == nil && p.options.notEmpty && nDst == 0 {
		return nDst, nSrc, errors.New("enforce resulted in empty string")
	}
...
func (c checker) Transform(dst, src []byte, atEOF bool) (nDst, nSrc int, err error) {
	for nSrc < len(src) {
		...
		if c.p.Allowed().Contains(r) {
			...
			return nDst, nSrc, errors.New("disallowed rune encountered")
		}
		nSrc += size
	}
